...
> 펌웨어 소스는 `firmware/`(빌드·플래시: `flash.sh`), 구설계 보존본은 `legacy/`(설치 금지)에 있다.
> 현행 펌웨어 호흡: 노랑(233,233,50), 포물선 근사, PULSE_MIN 16~255, 주기 ≈4.1 s(128×32 ms)
> — Pi 데몬(4.0 s 사인, 0.15~1.0)과 근사 일치. 인수가 항상 피크(최대 밝기)에서 일어나
> 체감 차이는 없으나, 호흡 스펙을 바꿀 땐 펌웨어·데몬·RAY 세 곳을 함께 맞출 것.
...
**렌더링 (현행 동작)**
- **단일 규칙 — 노란 호흡**: 데몬이 스트립을 잡고 있는 동안엔 항상 노란 호흡
  (233,233,50 · 4.0 s 사인 · 밝기 0.15~1.0, 정점에서 시작)을 그린다. ATtiny의 부팅
  호흡·RAY 대기 호흡과 동일 스펙이라 세 구간이 하나의 호흡으로 이어진다.
  RAY 인수 시엔 RAY 첫 프레임(bar 소등 + ring 0.15)으로 0.3 s 보간 페이드해 파킹한다.
- **반납 후엔 소등 유지**: RAY가 잡았다 놓으면 재획득까지 아무것도 그리지 않는다
  (개발 중 RAY를 꺼두면 LED도 꺼짐). 과거의 무지개 표시(150 s 폴백/"RAY 없음" 신호)는
  초기 인수 테스트용이었고 실운용에 불필요해 제거했다(2026-09).
...
**poweroff ACK 훅** (`pi/os-led-poweroff-ack` → `/usr/lib/systemd/system-shutdown/`)
— systemd가 종료 맨 끝(모든 서비스 정지 후)에 실행한다. 진짜 poweroff/halt면 READY(GPIO17)를
**300 ms 펄스**로 올려 ATtiny에게 "재부팅이 아니라 최종 종료"를 알린다 → ATtiny가 재부팅
대기 없이 즉시 링을 페이드아웃. 재부팅이면 아무것도 안 한다(데몬 재기동 시 READY가 다시
HIGH로 유지되는 것을 부팅 완료로 읽음). 펄스 폭은 펌웨어 ACK 윈도우 안이어야 한다:
...
```
RAY → "ACQUIRE\n" : 노란 호흡을 RAY 첫 프레임으로 0.3 s 보간 페이드 → SPI 쓰기 완전 중단
                    → "GRANTED\n". 이후 RAY가 직접 그림.
RAY → "RELEASE\n" : (또는 소켓 close = RAY 크래시 포함) 0.25 s settle 후 소등 유지.
                    재획득 시 검정 → RAY 첫 프레임(ring 0.15)으로 페이드.
```
...
동작 검증 순서:
1. 전원 인가 → fade-up + 호흡 → 부팅 완료 시 Pi가 노란 호흡 인계 → RAY 준비 시 RAY LED 직행
   (RAY 미설치/미기동이면 호흡이 계속 유지)
2. Pi off에서 터치 0.5 s → J2 펄스 부팅
3. Pi on에서 터치 2 s 연속 → 종료 호흡 → 소등
4. `sudo reboot` → 종료 호흡 → 부팅 호흡 (터치·J2 없이)
5. arbiter: 소켓에 ACQUIRE를 보내 데몬이 화면을 넘기는지, 소켓을 끊으면 소등되는지:
   ```python
   import socket, time
