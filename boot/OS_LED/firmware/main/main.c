...
/* Walk the breath from wherever it is to the parabola peak (PULSE_MAX) so a
 * following fade_down starts from 255 without a brightness jump. */
    ...
    while (elapsed < SELF_DOWN_TIMEOUT_MS) {
        ...
        if (pi_ready()) {
            ...
            if (high_ms >= READY_BOOT_HOLD_MS) {
                show(PULSE_MAX);   /* match daemon's handoff frame (full breath colour) */
