...
- **실시간 페이싱.** 100 ms(3200 B) 조각이 초당 약 10개 온다. TTS처럼 앞서 보내지 않는다. 말하지 않을 때도 무음 조각이 같은 속도로 계속 온다(전화선과 같음). 조각 크기·속도는 **관측치**로 문서 보증이 없다(WebSocket 가이드: "Output audio events have no timing fields"). 입력은 문서가 실시간을 요구한다("paced at its recorded sample rate").
- **총량은 실시간보다 적다 — 유실 (2026-09-16 확정).** `store: true` 세션의 서버 녹음(오른쪽 채널 = 출력)과 받은 오디오를 상호상관으로 대조: 받은 오디오는 생성분과 샘플 단위로 일치하고 빠진 구간만 녹음에 남는다(지연 아님). 유선·전부 무음 3분 97.3%(단일 프레임 누락 ~36회, 도착 간격 200 ms), 유선·90% 말소리 3분 99.8%(400 ms 한 번, 도착 간격 흔적 없음), Wi-Fi 7세션 96~98.5%. 유실은 무음에 집중되고, 말소리 유실은 드물지만 감지·복구 불가. 서버 제어 가이드 "Reflected output ranges can have gaps for dropped frames". 사이드밴드 타임스탬프 검증은 WS 프라이머리 세션에 attach 불가(404, WebRTC·SIP 전용)라 못 쓴다.
- 따라서 클라이언트에 지터 버퍼가 필수이고, 그 크기가 곧 추가 지연이다. 한 번 밀린 지연은 서버가 따라잡아 주지 않으므로 무음 조각을 버리는 등으로 스스로 줄여야 한다.
- 조각 간격 분포(Wi-Fi, 5분, 2,910개): 95%가 110 ms 이내, 200 ms 초과 31회(약 10초에 1회), 300~400 ms 17회, 최대 510 ms. 이후 여러 실행에서도 최대 500 ms 안팎. **400 ms 여유면 5분에 1회 끊김, 500 ms 넘게 잡아야 전부 흡수.** **유선(2026-09-16, 3분×3)**: 97.9%가 110 ms 이내, 최대 214~363 ms, 250 ms 초과 0~2회 — 정지는 Wi-Fi 구간이 원인.
...
- 첫 소리까지: `session.started`까지 1.5~3.5초. **모델은 스스로 인사하지 않는다** — 재촉 없이 아무도 말하지 않으면 180초 내내 무음(2026-09-16 run1). 초기 실험에서 "스스로 인사"로 보였던 것은 열린 마이크의 방 소리·사용자 말에 반응한 것. `session.commentary.append("Begin now …")`로 재촉하면 0.9초 만에 인사(run2). 세션 시작 직후 첫 0.2초에 진폭 ≤50의 미세 비영 샘플이 오므로 "말소리" 판정은 rms 기준이어야 한다.
