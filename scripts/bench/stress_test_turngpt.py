...
def build_incremental_inputs(
    ...
    *,
    ...
    for conv in conversations:
        if not chain:
            dialog_parts = []
        for _utt_idx, utterance in enumerate(conv):
...
def main():
    ...
    parser.add_argument(
        "--max-tokens", type=int, default=256, help="max_context_tokens (model trained on 256)"
    )
    parser.add_argument(
        "--keep-turns", type=int, default=2, help="Completed turns to keep after eviction"
    )
    ...
    model_path = (
        "models/turngpt/turngpt_v2_kvcache_int8.onnx"
        if args.int8
    ...
    while True:
        ...
        if (
            n_tokens > args.max_tokens * 0.8
            and prev_token_count > 0
            and n_tokens < prev_token_count
        ):
            eviction_count += 1
        ...
        if len(latencies) % 100 == 0:
            ...
            s = int(elapsed % 60)
            print(
                f"    [{m:02d}:{s:02d}] calls={len(latencies)}, "
    ...
    for threshold in [100, 150, 200]:
        ...
        if n_spikes > 1:
            ...
            gap_info = ""
        print(
            f"    >{threshold}ms : {n_spikes}/{len(lats)} ({pct:.1f}%)"
