...
def initialize_vad():
    ...
    try:
        ...
        vad_model = model
        vad_iterator = VADIterator(model, min_speech_duration_ms=100)
        logging.info(f"✅ Silero VAD 초기화 완료")
        return True
    except Exception as e:
            ...
            while True:
                try:
                    ...
                    if vad_iterator is None:
                        logging.error("VAD 반복자가 초기화되지 않았습니다.")
                        break
                    ...
                    if speech_dict and "start" in speech_dict:
                        logging.info("🗣️ 음성 시작 감지! 응답 중단 및 STT 시작...")
                        ...
                        # STT 실행
                        user_text = await asyncio.to_thread(run_stt, timeout_sec=30.0)
                        
                        # 처리 후 VAD 상태와 오디오 큐 초기화
                        vad_iterator.reset_states()
                        logging.info("VAD 상태를 초기화했습니다.")
                        while not audio_queue.empty():
                            audio_queue.get_nowait()
                        logging.info("오디오 큐를 비웠습니다.")
...
def run_stt(timeout_sec: float = 5.0):
    """
    ...
    try:
        while True:
            ...
            if ray_mode == "sleep":
                ...
                user_text = await asyncio.to_thread(run_stt, timeout_sec=5)
                if "레이" in user_text:
