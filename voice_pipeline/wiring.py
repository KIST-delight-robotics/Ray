...
from voice_pipeline.core.interfaces import ITTS, IStorageBackend, IToolExecutor
...
def build_components(
    *,
    ...
    Args:
        ...
        llm_tools: 응답 LLM에 활성화할 툴 이름 목록. ``None``이면 기본
            ``["web_search"]``.
        tool_executor: 함수 툴(예: ``control_light``) 실행기. ``None``이면
            함수 툴 호출을 무시한다 (eval 기본).
    ...
    llm = OpenAILLM(model="gpt-5.4-mini", temperature=0.7, reasoning_effort="none", max_tokens=256, tools=llm_tools)
