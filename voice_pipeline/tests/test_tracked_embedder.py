...
class TestTrackedEmbedder:
    ...
    def test_embed_batch_passthrough(self, setup: tuple[TrackedEmbedder, RecordingCallStore, _FakeEmbedder]) -> None:
        wrapper, _, _ = setup
        result = wrapper.embed_batch(["a", "b", "c"])
        assert result.shape == (3, 4)

    def test_embed_records_call(self, setup: tuple[TrackedEmbedder, RecordingCallStore, _FakeEmbedder]) -> None:
    ...
    def test_embed_batch_records_metadata(
        self, setup: tuple[TrackedEmbedder, RecordingCallStore, _FakeEmbedder]
    ) -> None:
        wrapper, store, _ = setup
    ...
    def test_dimension_delegates(self, setup: tuple[TrackedEmbedder, RecordingCallStore, _FakeEmbedder]) -> None:
        wrapper, _, _ = setup
        assert wrapper.dimension == 4

    def test_model_name_delegates(self, setup: tuple[TrackedEmbedder, RecordingCallStore, _FakeEmbedder]) -> None:
        wrapper, _, _ = setup
