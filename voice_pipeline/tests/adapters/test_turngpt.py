...
class TestInit:
    ...
    def test_load_file_not_found_raises_turngpt_error(self, _inject_turngpt_module, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="Failed to load TurnGPT model"):
            TurnGPTWrapper()
...
class TestErrorHandling:
    ...
    def test_import_error_raises_turngpt_error(self, monkeypatch):
        """If turngpt package is missing, constructor raises RuntimeError."""
        import sys
        ...
        try:
            sys.modules.pop("voice_pipeline.adapters.turngpt", None)
            from voice_pipeline.adapters.turngpt import TurnGPTWrapper
...
class TestOnnxInit:
    ...
    def test_missing_tokenizer_path_raises_turngpt_error(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="tokenizer path must not be empty"):
            TurnGPTWrapper()
...
class TestExtractTrpNumpy:
    ...
    def test_returns_eos_probability(self):
        from voice_pipeline.adapters.turngpt import _extract_trp_numpy
...
class TestBuildSpeakerIds:
    ...
    def test_single_turn_all_speaker1(self):
        from voice_pipeline.adapters.turngpt import _build_speaker_ids
