...
class TestASRStress:
    def test_rapid_reset_cycles(self, asr_wav: Path, asr_lang: str) -> None:
    ...
    def test_sustained_streaming(self, asr_wav: Path, asr_lang: str) -> None:
    ...
    def test_back_to_back_start_stop(self, asr_wav: Path, asr_lang: str) -> None:
    ...
    def test_reset_without_feeding_audio(self, asr_wav: Path, asr_lang: str) -> None:
        ...
        """
        info, _ = read_wav_frames(asr_wav)
        asr = make_asr_for_wav(info, asr_lang)
        asr.start()
        try:
            for _ in range(5):
