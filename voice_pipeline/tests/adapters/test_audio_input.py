"""Tests for voice_pipeline.adapters.audio_input."""
...
class TestAudioInputImport:
    def test_import_error_raises(self) -> None:
        """Missing pyaudio raises RuntimeError at construction."""
        ...
        try:
            ...
            import voice_pipeline.adapters.audio_input as mod

            importlib.reload(mod)

            with pytest.raises(RuntimeError, match="PyAudio is not installed"):
...
class TestMultiChannelExtract:
    ...
    def test_extract_channel_out_of_range_raises(self, monkeypatch) -> None:
        """extract_channel >= capture_channels should raise RuntimeError."""
        ...
        assert isinstance(ai._error, RuntimeError)
