...
from voice_pipeline.tests.adapters.conftest import (
    FakeStreamEvent,
...
def _build_llm(mock_client: MagicMock, **kwargs) -> OpenAILLM:
    ...
    with patch("voice_pipeline.adapters.llm_openai.openai.OpenAI", return_value=mock_client):
...
class TestGenerate:
    def test_streaming_yields_text_chunks(self) -> None:
        chunks = ["Hello", ", ", "world!"]
        client = create_mock_llm_client(make_stream_events(chunks))
    ...
    def test_non_text_events_ignored(self) -> None:
        ...
        client = create_mock_llm_client(events)
        llm = _build_llm(client)
...
class TestSystemMessageExtraction:
    def test_system_message_routed_to_instructions(self) -> None:
        client = create_mock_llm_client(make_stream_events(["ok"]))
        llm = _build_llm(client)
...
class TestErrorHandling:
    ...
    def test_unexpected_streaming_exception_wrapped(self) -> None:
        """Non-OpenAI exception during streaming is wrapped in RuntimeError."""
        client = create_mock_llm_client()
        mock_stream = client.responses.create.return_value
...
class TestStreamCleanup:
    def test_stream_closed_after_full_iteration(self) -> None:
        client = create_mock_llm_client(make_stream_events(["a", "b"]))
        llm = _build_llm(client)
    ...
    def test_stream_closed_on_partial_iteration(self) -> None:
        ...
        client = create_mock_llm_client(make_stream_events(["a", "b", "c"]))
        llm = _build_llm(client)
...
class TestCompletedResponsePayload:
    ...
    def test_metrics_from_object_response(self) -> None:
        from voice_pipeline.tests.adapters.conftest import FakeCompletedResponse, FakeUsage, FakeUsageDetails
        ...
        client = create_mock_llm_client(make_stream_events(["hi"], completed_response=resp))
