"""Tests for voice_pipeline.adapters.asr_google (GoogleCloudASR)."""
...
@patch("voice_pipeline.adapters.asr_google.speech.SpeechClient")
class TestStartStop:
...
class TestEdgeCases:
    ...
    def test_unsupported_sample_width(self, mock_client_cls: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("voice_pipeline.adapters.asr_google.SAMPLE_WIDTH", 4)
        asr = _make_asr()

        with pytest.raises(RuntimeError, match="sample_width=4"):
            asr.start()
    ...
    def test_sample_rate_too_low(self, mock_client_cls: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("voice_pipeline.adapters.asr_google.SAMPLE_RATE", 4000)
        asr = _make_asr()

        with pytest.raises(RuntimeError, match="sample_rate=4000"):
            asr.start()
    ...
    def test_sample_rate_too_high(self, mock_client_cls: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("voice_pipeline.adapters.asr_google.SAMPLE_RATE", 96000)
        asr = _make_asr()

        with pytest.raises(RuntimeError, match="sample_rate=96000"):
            asr.start()
    ...
    def test_sample_rate_boundary_valid(self, mock_client_cls: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        for rate in (8000, 48000):
            monkeypatch.setattr("voice_pipeline.adapters.asr_google.SAMPLE_RATE", rate)
            asr = _make_asr()
            asr.start()
            asr.stop()
