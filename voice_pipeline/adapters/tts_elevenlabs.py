...
from voice_pipeline.types import ITTS, TTSStream, WordTimestamp
...
class ElevenLabsTTS(ITTS):
    ...
    Reads ``ELEVENLABS_API_KEY`` from the environment; raises :class:`RuntimeError`
    ...
    def synthesize(self, text: str) -> TTSStream:
        ...
        Raises:
            RuntimeError: On validation failure or API error. The SDK issues the
    ...
    def _validate_input(self, text: str) -> None:
        ...
        if not text or not text.strip():
            raise RuntimeError("Text must not be empty or whitespace-only")
        if len(text) > self._MAX_TEXT_LEN:
            raise RuntimeError(f"Text exceeds {self._MAX_TEXT_LEN} character limit ({len(text)} chars)")
...
def _iter_chunks(
    ...
    try:
        ...
        raise RuntimeError(f"ElevenLabs API error (status={exc.status_code}): {body}") from exc
    except httpx.TimeoutException as exc:
        logger.error("ElevenLabs TTS timeout: %s", exc)
        raise RuntimeError(f"TTS timeout: {exc}") from exc
