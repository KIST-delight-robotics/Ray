"""VAP(Voice Activity Projection) 어댑터.

- ``MaAIVAPModel``: ONNX 인코더 + 트랜스포머 동기 추론. 스레드 안전하지 않음.
  ``_USE_ONNX_TRANSFORMER = False`` 면 PyTorch(``maai`` 패키지, external/MaAI) 폴백.
- ``ThreadedVAP``: 모델을 전용 스레드에서 돌리는 파이프라인용 런타임. ``feed_audio`` 는 버퍼링만 하고,
  스레드가 frame_rate Hz로 추론해 ``latest_result`` 를 갱신한다. 프로세스 수명 객체 — 세션마다
  ``reset()`` 과 ``session_id`` 재지정으로 재사용, 종료 시 ``stop()``.

RPi 5에서는 ort_threads=1 이 가장 빠르다. 셋업: docs/modules/turn_taking.md
"""
...
from voice_pipeline.settings import SAMPLE_RATE

logger = logging.getLogger("voice_pipeline.adapters.vap")
...
class MaAIVAPModel:
    ...
    def __init__(
        ...
        if not self.ENCODER_ONNX_PATH:
            raise RuntimeError("ENCODER_ONNX_PATH is required")
        if not os.path.isfile(self.ENCODER_ONNX_PATH):
            raise RuntimeError(f"Encoder ONNX file not found: {self.ENCODER_ONNX_PATH}")
        try:
            ...
            raise RuntimeError(f"Failed to load ONNX encoder: {exc}") from exc

        # Transformer
        if self._use_onnx_transformer:
            if not os.path.isfile(self.TRANSFORMER_ONNX_PATH):
                raise RuntimeError(f"Transformer ONNX file not found: {self.TRANSFORMER_ONNX_PATH}")
            try:
                ...
                raise RuntimeError(f"Failed to load ONNX transformer: {exc}") from exc
        else:
            # PyTorch transformer requires torch
            if torch is None:
                raise RuntimeError("torch is required when _USE_ONNX_TRANSFORMER is False")
            ...
            try:
                ...
                raise RuntimeError(f"Failed to load MaAI: {exc}") from exc
...
class ThreadedVAP:
    """ThreadedVAP runtime that runs a VAP model on a background thread.
