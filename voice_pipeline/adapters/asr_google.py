...
from voice_pipeline.types import IASR, AudioFrame
...
class GoogleCloudASR(IASR):
    ...
    def _start_stream(self) -> None:
        ...
        if not (8000 <= SAMPLE_RATE <= 48000):
            raise RuntimeError(f"sample_rate={SAMPLE_RATE} outside Google STT valid range [8000, 48000]")

        encoding = self._ENCODING_MAP.get(SAMPLE_WIDTH)
        if encoding is None:
            raise RuntimeError(
                f"Unsupported sample_width={SAMPLE_WIDTH}; supported: {sorted(self._ENCODING_MAP.keys())}"
            )
