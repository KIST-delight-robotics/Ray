...
class CppBridge:
    ...
    def connect(self) -> None:
        ...
        for attempt in range(1, self._RECONNECT_ATTEMPTS + 1):
            ...
            raise RuntimeError(f"Failed to connect to {uri} after {self._RECONNECT_ATTEMPTS} attempts") from last_exc
    ...
    def _send_json(self, msg: dict) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Connection lost during send: {exc}") from exc
        except WebSocketException as exc:
            raise RuntimeError(f"WebSocket error during send: {exc}") from exc
    ...
    def _receive_loop(self, conn: ClientConnection, stop: threading.Event) -> None:
        ...
        while not stop.is_set():
            try:
                ...
                if not stop.is_set():
                    with self._lock:
                        self._error = RuntimeError(f"Connection lost: {exc}")
                    self._running.clear()
                return
            except Exception as exc:
                if not stop.is_set():
                    with self._lock:
                        self._error = RuntimeError(f"Receiver error: {exc}")
                    self._running.clear()
                return
