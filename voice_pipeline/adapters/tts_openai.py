...
class OpenAITTS(ITTS):
    ...
    def synthesize(self, text: str) -> TTSStream:
        ...
        Raises:
            RuntimeError: On validation failure or API error.
        """
        ...
        try:
            ...
            raise RuntimeError(f"TTS timeout ({self._TIMEOUT_SEC}s): {exc}") from exc
    ...
    def _validate_input(self, text: str) -> None:
        ...
        if len(text) > 4096:
            raise RuntimeError(f"Text exceeds 4096 character limit ({len(text)} chars)")
        if not (0.25 <= self._SPEED <= 4.0):
            raise RuntimeError(f"Speed must be 0.25–4.0, got {self._SPEED}")
