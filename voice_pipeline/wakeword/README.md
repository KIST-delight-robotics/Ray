...
```python
from voice_pipeline.wakeword import WakewordDetector
...
uv run pytest voice_pipeline/tests/wakeword/test_wakeword.py -v

# Integration tests (real models + API)
WAKEWORD_TEST_WAV=path/to/wakeword.wav \
  uv run pytest voice_pipeline/tests/wakeword/test_wakeword_integration.py -v -m "requires_api and requires_model"
```
