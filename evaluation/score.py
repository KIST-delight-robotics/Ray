...
Usage:
    uv run python -m evaluation.report var/eval/results
    uv run python -m evaluation.score var/eval/results/report.json
"""
...
def score_report(report: dict) -> dict:
    ...
    for turn in report["turns"]:
        ...
        # Pipeline latency collection (text mode 제외 — 텍스트 턴의 latency 키는
        # 음성 파이프라인 지표와 다른 축이라 분포에 섞이면 안 됨)
        if not is_text_mode:
